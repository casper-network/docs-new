...
If it fails for any reason, please refer the [NCTL tutorial](https://docs.casperlabs.io/en/latest/developers/prerequisites-nctl.html) and make sure that all your packages are up to date).
...
With the compiled contract, we can call the _casper-client put-deploy_ command to put the contract on the chain.
