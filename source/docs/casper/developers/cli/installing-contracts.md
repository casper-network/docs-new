...
- You know how to [send and verify deploys](/developers/dapps/sending-deploys.md)
   ...
   - You have enough CSPR tokens in your account's main purse to pay for deploys. If you plan to use the Casper Testnet, learn about the [faucet](/workflow/users/token-transfer#2-the-faucet) to fund your testing account's main purse
- You understand how to [write basic contract code](/developers/writing-contracts/index.md) and session code
...
Here is how the contract package details would look. The response would contain the `contract_hash`, which you would need to [call a contract by hash](/developers/cli/calling-contracts.md#calling-contracts-by-hash) in the next section. You would also see the `access_key` for the `ContractPackage` and the current `contract_version`.
...
- Learn [different ways to call contracts](/developers/cli/calling-contracts.md) using the Casper command-line client
