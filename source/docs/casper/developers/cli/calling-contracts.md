...
- You know how to [install contracts and query global state](/developers/cli/installing-contracts.md) using the [default Casper client](/developers/prerequisites#the-casper-command-line-client)
...
After [installing a contract](/developers/cli/installing-contracts.md) in global state, you can use the contract's hash to call one of its entry points. The following usage of `put-deploy` allows you to call an entry point and receive a deploy hash. The hash is needed to verify that the deploy was processed successfully.
...
This `put-deploy` command is nearly identical to the command used to [install the contract](/developers/cli/installing-contracts.md#installing-contract-code). Here, instead of `session-path` pointing to the Wasm binary, we have `session-hash` and `session-entry-point` identifying the on-chain contract and its associated entry point. No Wasm file is needed in this example since the contract is already on the blockchain, and the entry point doesn’t return a value. If an entry point returns a value, use code to [interact with runtime return values](/resources/tutorials/advanced/return-values-tutorial/).

:::

The following sample response contains a `deploy_hash`, needed to verify the changes in global state, as described [here](/developers/cli/installing-contracts.md#querying-global-state).
...
The sample response will contain a `deploy_hash`, which you need to use as described [here](/developers/cli/installing-contracts.md#querying-global-state) to verify the changes in global state.
...
The [Counter Contract Tutorial](/resources/tutorials/beginner/counter/index.md) shows how to change the state of a contract (counter-define.wasm) using session code (counter-call.wasm).
...
Visit the [Interacting with Runtime Return Values](/resources/tutorials/advanced/return-values-tutorial/) tutorial to learn how to call a contract that returns a value using session code or contract code.
...
- Learn more about [Delegating with the Casper Client](/developers/cli/delegate)
- Look into the [Tutorials for Smart Contract Authors](/resources/tutorials/)
- See the [Developer How To Guides](/developers/table-of-content)
