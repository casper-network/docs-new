...
This is the only means by which users can send their compiled Wasm (as part of a Deploy) to a node on a Casper network. The request takes in the [Deploy](/concepts/deploy-and-deploy-lifecycle.md#execution-semantics-deploys) as a parameter, prior to sending it to a node on a network for execution.

|Parameter|Type|Description|
|---------|----|-----------|
|[deploy](/developers/json-rpc/types_chain.md#deploy)|Object|A Deploy consists of an item containing a smart contract along with the requester's signature(s).|

> **Note**: You can find a list of [trusted peers](/operators/becoming-a-validator/joining/#known-addresses) in the network's configuration file, `config.toml`. Here is an [example config.toml](https://github.com/casper-network/casper-node/blob/dev/resources/production/config-example.toml#L131). You may send deploys to one of the trusted nodes or use them to query other online nodes.

### `account_put_deploy_result`

The result contains the [deploy_hash](/developers/json-rpc/types_chain.md#deployhash), which is the primary identifier of a Deploy within a Casper network.

|Parameter|Type|Description|
|---------|----|-----------|
|api_version|String|The RPC API version.|
|[deploy_hash](/developers/json-rpc/types_chain.md#deployhash)|String| A hex-encoded hash of the Deploy as sent.|
...
|Parameter|Type|Description|
|---------|----|-----------|
|[block_identifier](/developers/json-rpc/types_chain.md#blockidentifier)|Object|The block hash or height on top of which to execute the deploy. If not supplied,the most recent block will be used.|
|[deploy](/developers/json-rpc/types_chain.md#deploy)|Object|A Deploy consists of an item containing a smart contract along with the requester's signature(s).|
...
|---------|----|-----------|
|api_version|String|The RPC API version.|
|[block_hash](/developers/json-rpc/types_chain.md#blockhash)|Object|The Block hash on top of which the deploy was executed.|
|[execution_results](/developers/json-rpc/types_chain.md#executionresult)|Object|The map of Block hash to execution result.|