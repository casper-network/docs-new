...
Casper is a Proof-of-Stake blockchain platform with an account-based model that performs execution after consensus. A Casper network stores data on a structure known as [Global State](#global-state-head). Users interact with global state through session code sent in a [Deploy](/concepts/deploy-and-deploy-lifecycle.md#execution-semantics-deploys). Deploys contain [Wasm](https://webassembly.org/) to be executed by the network, thus allowing developers to use their preferred programming language rather than a proprietary language.
...
Refer to [Keys and Permissions](/concepts/design/serialization-standard.md#serialization-standard-state-keys) for further information on keys.
...
- [ed25519](/concepts/accounts-and-keys.md#eddsa-keys) keys, which use the Edwards-curve Digital Signature Algorithm (EdDSA) and are 66 bytes long
- [secp256k1](/concepts/accounts-and-keys.md#ethereum-keys) keys, commonly known as Ethereum keys, which are 68 bytes long
...
The [block header](/concepts/design/serialization-standard/#serialization-standard-block) contains the following fields:
...
Refer to the [Serialization Standard](/concepts/design/serialization-standard.md) for additional information on how blocks and deploy are serialized.
