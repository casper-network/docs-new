...
- [Setting up the Casper client](/developers/prerequisites/#the-casper-command-line-client)
...
- [Setting up an Account](/developers/prerequisites/#setting-up-an-account)
...
refund_handling = { type = "refund", refund_ratio = [1, 1] }
```
After deducting the gas fee, the distribution of the remaining payment amount is handled based on the [fee_handling](/operators/setup-network/create-private/#fee-handling-config) configuration.
