...
- You should be familiar with [writing smart contracts](/developers/writing-contracts), [on-chain contracts](/developers/dapps/sending-deploys/), and [calling contracts](/developers/cli/calling-contracts) on a Casper network
...
[Install the contract](/developers/dapps/sending-deploys/#sending-the-deploy) on the network via a deploy and verify the deploy status. You can also [monitor the event stream](/developers/dapps/sending-deploys/#monitoring-the-event-stream-for-deploys) to see when your deploy is accepted.
