...
You can determine the type of the return value by [querying the contract object](/resources/tutorials/beginner/querying-network/#querying-an-account) in global state. To query a contract rather than an account, replace the key parameter with the formatted string representation of the contract hash.
