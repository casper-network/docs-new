...
The [block proposal](/glossary/B.md#block-proposal) happens first, and the proposed [proto block](/glossary/P.md#proto-block) contains a set of deploys that have not been executed yet.

Only after consensus on a _proto block_ has been reached, the deploys are executed. The resulting new global state [root hash](/glossary/R.md#root-hash) is put into an actual block, together with the executed deploys.
...
Blockchain is a P2P network where the collection of nodes ([validators](/glossary/V.md#validator)) concurrently updates a decentralized, shared database. They do this collectively, building an ever-growing chain of [transactions](/glossary/T.md#transaction). For performance reasons, transactions are bundled in [blocks](#block). According to a particular cooperation protocol (consensus protocol), the collection of [nodes](/glossary/N.md#node) connected via a P2P network cooperate to maintain this shared database as a single source of truth. The database's current state is called the [global state](/glossary/G.md#global-state) and has a sizeable map-like collection.
...
The amount of money (in crypto-currency) that is allocated by a node in order to participate in [consensus](/glossary/C.md#consensus) (and to be a [validator](/glossary/V.md#validator)).

## Bonding {#bonding}

Depositing money in the [auction contract](/glossary/A.md#auction-contract) and try to become a [staker](/glossary/S.md#staker). The bonding request is a transaction that transfers tokens to the auction contract. In the next [booking block](#booking-block), a new set of validators is determined, with weights according to their deposits. This new set becomes active in the era(s) using that booking block.
