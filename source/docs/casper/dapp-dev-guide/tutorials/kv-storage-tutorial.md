...
You can run unit tests locally if you have set up the contract using _cargo-casper_ as shown in the [Getting started](/developers/writing-onchain-code/getting-started.md) guide.
...
The internal state of the blockchain is updated via a series of steps (blocks). All blockchain queries must include a [_global state hash_ which corresponds to the block hash or height of the blockchain. Visit [Querying the address of a contract](/developers/writing-onchain-code/getting-started.md#querying-global-state-for-the-address-of-a-contract).
...
Please reference the [Querying Section](/developers/writing-onchain-code/getting-started.md#querying-global-state-for-the-address-of-a-contract) for more details.
